...
class ThreatIntelligenceScanner:
    def __init__(self):
        ...
        self.indicators = {
            ...
            'crackmapexec': r'(?i)crackmapexec',
            'impacket': r'(?i)impacket',
            'rdp_enable': r'(?i)netsh.*rdp',
            'smbclient': r'(?i)smbclient',
            'winexe': r'(?i)winexe',
            'at_task': r'(?i)at.exe',
            ...
            'conti_ransom': r'(?i)conti',
            'ryuk_ransom': r'(?i)ryuk',
            'reevil': r'(?i)reevil',
            'babuk': r'(?i)babuk',
            'darkside': r'(?i)darkside',
            'avaddon': r'(?i)avaddon',
            # --- Cloud & Identity ---
            ...
            'jsp_shell': r'(?i)runtime.*exec',
            'asp_shell': r'(?i)wscript.shell',
            'icmp_beacon': r'(?i)icmp.*beacon',
            'dns_beacon': r'(?i)dns.*beacon',
            'smb_beacon': r'(?i)smb.*beacon',
            'malleable_c2': r'(?i)malleable.*profile',
            'nmap_scan': r'(?i)nmap',
            'masscan': r'(?i)masscan',
            'adfind': r'(?i)adfind',
            'zerologon': r'(?i)zerologon',
            'printnightmare': r'(?i)printnightmare',
            'log4j': r'(?i)jndi:ldap',
            'solarwinds': r'(?i)solarwinds',
            'proxylogon': r'(?i)proxylogon',
            'hafnium': r'(?i)hafnium',
            'living_off_the_land': r'(?i)lolbin',
            'fileless_malware': r'(?i)fileless',
...
try:
    ...
    if folder_path:
        ...
        print(json.dumps({"error": "No directory selected"}))
except Exception as e:
    print(json.dumps({"error": str(e)}))
"""
    try:
        result = subprocess.run([sys.executable, "-c", picker_code], capture_output=True, text=True, timeout=60)
        if result.returncode != 0:
            st.error(f"Folder picker failed: {result.stderr}")
            return None
        output = result.stdout.strip()
        if not output:
            st.error("No output from folder picker")
            return None
        data = json.loads(output)
        if "error" in data:
            st.error(f"Error selecting folder: {data['error']}")
            return None
...
def filter_findings(findings, selected_file):
    if selected_file == "All":
        return findings
    filtered_findings = {}
    for indicator_type, data in findings.items():
        matches = data.get('data', data.get('sample', []))
        filtered_matches = [m for m in matches if m['file'] == selected_file]
        if filtered_matches:
        ...
        if st.session_state.selected_directory and st.sidebar.button("🔍 Scan Directory", type="primary"):
            if os.path.exists(st.session_state.selected_directory) and os.path.isdir(st.session_state.selected_directory):
                ...
                for pat in patterns_list:
                    files_to_scan.extend(glob(os.path.join(directory_path, '**', pat), recursive=True))
                files_to_scan = [f for f in files_to_scan if os.path.isfile(f)]
                if not files_to_scan:
                    st.error("No files found to scan!")
                else:
                    total_files = len(files_to_scan)
                    st.info(f"Found {total_files} files to scan")
                    progress_bar = st.progress(0)
                    status_text = st.empty()
                    all_findings = defaultdict(list)
                    scanned = 0
                    for file_path in files_to_scan:
                        display_name = os.path.relpath(file_path, directory_path)
                        findings = scanner.scan_file(file_path, display_name=display_name)
                        for ind, matches in findings.items():
                            all_findings[ind].extend(matches)
                        scanned += 1
                        progress_bar.progress(scanned / total_files)
...
def display_indicators_overview(findings):
    ...
    categories = {
        ...
        'Common Malware': ['mimikatz', 'metasploit', 'cobalt_strike', 'empire', 'beacon', 'nishang',
                         'covenant', 'poshc2', 'sqlmap', 'nikto', 'bloodhound', 'sharphound', 'merlin_c2', 'starkiller'],
        ...
        'Ransomware': ['vssadmin_delete', 'wbadmin_delete', 'bcedit_recovery', 'ransom_note_readme',
                      ...
                      'play_ext', 'clop_ext', 'conti_ransom', 'ryuk_ransom', 'reevil', 'babuk', 'darkside', 'avaddon'],
        ...
        'Web Shells': ['web_shell_passthru', 'web_shell_shell_exec', 'web_shell_system', 'web_shell_cmd_asp',
                      'web_shell_aspx_cmd', 'china_chopper_ua', 'weevely', 'eval_base64', 'jsp_shell', 'asp_shell'],
        'Lateral Movement': ['psexec', 'wmic_remote', 'winrm', 'wmiexec', 'dcom_exec',
                            'smb_exec', 'evil_winrm', 'crackmapexec', 'impacket', 'rdp_enable', 'smbclient',
                            'winexe', 'at_task', 'sc_create'],
        ...
        'Command & Control': ['reverse_shell', 'bind_shell', 'powershell_iex', 'http_beacon', 'meterpreter',
                              'icmp_beacon', 'dns_beacon', 'smb_beacon', 'malleable_c2'],
        ...
        'Exfiltration': ['rclone', 'dnscat2', 'curl_upload', 'pastebin', 'anonfiles', 'mega_nz', '7zip_compress',
                         'scp_exfil', 'sftp_exfil', 'ftp_exfil', 'rsync_exfil', 'dropbox_api', 'onedrive_api'],
        ...
        'Malicious Programs': ['nmap_scan', 'masscan', 'adfind', 'zerologon', 'printnightmare', 'log4j',
